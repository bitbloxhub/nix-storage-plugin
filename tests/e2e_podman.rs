...
fn nix_build_output_with_retry(flake_ref: &str) -> std::process::Output {
	let max_attempts = 2;
	...
	last_output.expect("nix build attempts should produce output")
}

fn nix_out_path(flake_ref: String) -> PathBuf {
	let output = nix_build_output_with_retry(&flake_ref);

	if !output.status.success() {
		panic!(
			"nix build failed for {flake_ref}\nstdout:\n{}\n\nstderr:\n{}",
	...
	let stdout = String::from_utf8(output.stdout)
		.unwrap_or_else(|error| panic!("nix output was not valid UTF-8: {error}"));
	...
	stdout
		...
		.map(PathBuf::from)
		.next()
		.unwrap_or_else(|| panic!("nix build returned no output paths for {flake_ref}"))
}
...
fn pull_image_with_retry(env: &E2eEnv, image: &str) -> std::process::Output {
	let max_attempts = 3;
	let mut last_output = None;
	for attempt in 1..=max_attempts {
		let output = env.run_command(
			podman_bin(),
			&[OsString::from("pull"), OsString::from(image)],
		);
		if output.status.success() {
			return output;
		}
	...
	last_output.expect("pull attempts should produce output")
}
...
fn podman_runs_nix_image() {
	...
	let pull_output = pull_image_with_retry(&env, &image);
